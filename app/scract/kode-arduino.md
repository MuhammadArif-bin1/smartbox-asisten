...
// WIFI + MQTT CONFIG
// ==========================================================
#define FIRMWARE_VERSION "SMARTBOX_BAGUS_SYNC_V2"
...
// PIR Greeting state dari dashboard
bool pirGreetingEnabled = false;
int pirGreetingTrack = 1;
String pirGreetingStart = "07:00";
String pirGreetingEnd = "22:00";
...
void setRelay(uint8_t relay, bool state, const char *source);
bool isTimeWithinRange(const String &startStr, const String &endStr);
...
void checkRelaySchedules() {
  ...
  if (now - lastScheduleCheck < 10000) return;
  ...
  for (int i = 0; i < scheduleCount; i++) {
    ...
    bool shouldBeOn = isTimeWithinRange(schedules[i].start, schedules[i].end);
    
    if (schedules[i].relay == 1) {
      if (shouldBeOn && !relay1State) {
        setRelay(1, true, "schedule");
        publishEvent("INFO", "relay1.schedule_on", "Stop Kontak 1 menyala sesuai jadwal.");
      } else if (!shouldBeOn && relay1State && !relay1AutoOffActive) {
        setRelay(1, false, "schedule");
        publishEvent("INFO", "relay1.schedule_off", "Stop Kontak 1 mati sesuai jadwal.");
      }
    } else if (schedules[i].relay == 2) {
      if (shouldBeOn && !relay2State) {
        setRelay(2, true, "schedule");
        publishEvent("INFO", "relay2.schedule_on", "Stop Kontak 2 menyala sesuai jadwal.");
      } else if (!shouldBeOn && relay2State && !relay2AutoOffActive) {
        setRelay(2, false, "schedule");
        publishEvent("INFO", "relay2.schedule_off", "Stop Kontak 2 mati sesuai jadwal.");
      }
    }
  }
}
...
void handlePirVoice() {
  ...
  if (pirNow && !lastPirState) {
    ...
    if (now - lastPirVoiceAt >= PIR_VOICE_COOLDOWN_MS) {
      ...
      // Check if scheduled greeting is active
      if (pirGreetingEnabled && isTimeWithinRange(pirGreetingStart, pirGreetingEnd)) {
        playVoice(pirGreetingTrack, "pir_greeting_scheduled");
      } else {
...
// ==========================================================
// STATUS PAYLOAD BUILDER (for MQTT Last-Will)
// ==========================================================
String buildStatusPayload(bool online) {
  JsonDocument doc;
...
void publishTelemetry() {
  ...
  doc["temperatureHigh"] = (temperatureC >= TEMP_WARNING_THRESHOLD);
  ...
  doc["bluetoothAudio"] = bluetoothState;
  doc["ampRelay"] = bluetoothState;
  doc["buzzer"] = buzzerState;

  // Sensor Settings
  doc["gasSensorEnabled"] = gasSensorEnabled;
  doc["gasEnabled"] = gasSensorEnabled;
  doc["tempSensorEnabled"] = tempSensorEnabled;
  doc["tempEnabled"] = tempSensorEnabled;
  doc["pirSensorEnabled"] = pirSensorEnabled;
  doc["pirEnabled"] = pirSensorEnabled;
  doc["sleepModeEnabled"] = sleepModeEnabled;

  // PIR Greeting configs
  ...
  // Add schedules to telemetry
  JsonArray relaySchedulesArr = doc["relaySchedules"].to<JsonArray>();
  for (int i = 0; i < scheduleCount; i++) {
    JsonObject schObj = relaySchedulesArr.add<JsonObject>();
    schObj["id"] = schedules[i].id;
    schObj["relay"] = schedules[i].relay;
    schObj["enabled"] = schedules[i].enabled;
    schObj["timeRange"] = schedules[i].start + "-" + schedules[i].end;
  }
...
void mqttCallback(char *topic, byte *payload, unsigned int length) {
  ...
  if (strcmp(type, "voice.play") == 0) {
    ...
    gasSensorEnabled = data["enabled"] | true;
    publishAck(cmdId, type, true, "Sensor gas updated.");
  }

  else if (strcmp(type, "tempSensor.set") == 0) {
    tempSensorEnabled = data["enabled"] | true;
    publishAck(cmdId, type, true, "Sensor suhu updated.");
  }

  else if (strcmp(type, "pirSensor.set") == 0) {
    pirSensorEnabled = data["enabled"] | true;
    publishAck(cmdId, type, true, "Sensor PIR updated.");
  }

  else if (strcmp(type, "sleepMode.set") == 0) {
    sleepModeEnabled = data["enabled"] | false;
    publishAck(cmdId, type, true, "Sleep mode updated.");
  }

  else if (strcmp(type, "pirGreeting.set") == 0) {
    pirGreetingEnabled = data["enabled"] | false;
    pirGreetingTrack = data["track"] | 1;
    pirGreetingStart = data["start"] | "07:00";
    pirGreetingEnd = data["end"] | "22:00";
    publishAck(cmdId, type, true, "PIR Greeting updated.");
  }

  else if (strcmp(type, "relaySchedule.set") == 0) {
    String schId = data["id"] | "";
    int relayNum = data["relay"] | 1;
    String startStr = data["start"] | "00:00";
    String endStr = data["end"] | "00:00";
    bool enabled = data["enabled"] | false;

    int foundIdx = -1;
    for (int i = 0; i < scheduleCount; i++) {
      if (schedules[i].id == schId) {
        foundIdx = i;
        break;
      }
    }
    ...
    if (foundIdx != -1) {
      schedules[foundIdx].relay = relayNum;
      schedules[foundIdx].start = startStr;
      schedules[foundIdx].end = endStr;
      schedules[foundIdx].enabled = enabled;
    } else if (scheduleCount < MAX_SCHEDULES) {
      schedules[scheduleCount] = {schId, relayNum, startStr, endStr, enabled};
      scheduleCount++;
    }
    ...
    if (foundIdx != -1) {
      for (int i = foundIdx + 1; i < scheduleCount; i++) {
...
void connectWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    wifiReady = false;
    Serial.println("[WIFI ERROR] Gagal connect ke WiFi.");
    setLcdOverride("WIFI ERROR", "CEK SSID BAGUS", 3000);
    setRGB(255, 0, 0);
  }
}
...
void connectMQTT() {
  ...
  if (connected) {
    ...
    setLcdOverride("FW BAGUS", "SYNC V2", 4000);
    setRGB(0, 255, 0);
  } else {
    mqttReadyFlag = false;
...
void setup() {
  ...
  Serial.println("FIRMWARE VERSION: SMARTBOX_BAGUS_SYNC_V2");
  Serial.println("WIFI TARGET: BAGUS");
  Serial.println("BACKEND TARGET IP: 192.168.1.4");
  Serial.println("======================================");
