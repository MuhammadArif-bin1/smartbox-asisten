...
export async function POST(request: Request) {
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Jadwal alarm tidak valid. Nama wajib diisi, waktu harus format HH:MM, dan track harus 1-14." },
        { status: 400 }
      );
    }
    ...
    const newSchedule = await prisma.alarmSchedule.create({
      data: {
        ...
        buzzerActive: buzzerActive !== undefined ? Boolean(buzzerActive) : false,
